...
{$ENDIF}
  ...
  ZLib, BZlib, LZMADecomp, LZMADecompSmall,
...
begin
  ...
  VerIsUnicode := PosEx('(u)', s, Length(SignBegin)) > 0;
...
function GetBlockReaderClass() : TCompressedBlockReaderClass;
begin
  Result := nil;

  if (Ver <= 4008) then Result := TZlibBlockReader4008
  else if (Ver >= 4009) and (Ver <= 4105) then Result := TZlibBlockReader4107
  // version mismatch (4105 vs. 4107) is ok
  else if (Ver > 4105) then Result := Compress.TCompressedBlockReader;
end;
...
var
  ...
  begin
    { Wizard image }
    ReadString(Reader, WizardImage);

    if (Ver >= 2001) then
    begin
      ReadString(Reader, WizardSmallImage);
    end;
...
//  SetupFilename := SetupLdrOriginalFilename;
  ...
  try
    ...
    try
      TheBlockReader := GetBlockReaderClass();
...
begin
  ...
  try
    ...
    caInstallInfo:
      begin
        writeln('Compression used: '+GetCompressMethodName);
        Int64(TotalFileSize):=0; TotalFiles:=0; TotalEncryptedFiles:=0; MaxSlice:=0;
