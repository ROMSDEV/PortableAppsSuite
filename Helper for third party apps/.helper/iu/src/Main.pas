...
type
  ...
  TCommandAction=(caInstallInfo, {caListFiles,} caVerboseList, caExtractFiles, caWriteCode);
...
var
  ...
  WizardImage, WizardSmallImage, DecompDll:string;

procedure InternalError(const Id: String);
function TestPassword(const Password: String): Boolean;
function GetSliceName(ASlice:integer):string;
function FixPasswordEncoding(const Password: String) : String;
...
function AddFakeFile(FileName,FileContents:string; RenameNow:boolean=false):integer;
...
uses Windows, MD5, SHA1, PathFunc, MyTypes, CmnFunc2, Msgs, MsgIds;
...
function TestPassword_MD5(const Password: String; const PasswordHash: TSetupHash;
...
function TestPassword_SHA1(const Password: String; const PasswordHash: TSetupHash;
  const PasswordSalt: TSetupSalt): Boolean;
var
...
begin
  s:=StrToOem(s);
  WriteOffset:=0; BytesRemaining:=length(s);
  while BytesRemaining>0 do begin
    if BytesRemaining>1024 then BytesToWrite:=1024 else BytesToWrite:=BytesRemaining;
    WriteFile(StdOutputHandle,s[WriteOffset+1],BytesToWrite,BytesWritten,nil);
