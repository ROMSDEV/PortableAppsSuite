...
interface
uses
  MyTypes, Int64Em, structDEFVER_EXTSUF_;
...
const
  ...
  SetupHeaderOptionTable: array[0..MySetupHeaderOptionLast] of byte = (
    ...
    ord(structDEFVER_EXTSUF_.shChangesAssociations),
    ...
    {$IF DEFVER>=5004}ord(structDEFVER_EXTSUF_.shChangesEnvironment)        {$ELSE}255{$IFEND},
    {$IF (DEFVER>=5107) AND (NOT DEFINED(UNICODE))}
...
{$ELSE}
    ...
    InfoAfterText                 :='';
{$IFEND}
{$IF (DEFVER>=4202) AND not Defined(UNICODE)}
