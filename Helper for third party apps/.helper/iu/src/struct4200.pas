...

{
  ...
  $jrsoftware: issrc/Projects/Struct.pas,v 1.126 2004/03/19 20:39:18 jr Exp $
}
...
const
  ...
  SetupVersion = '4.2.0'  {$IFDEF USEBUILDTAG} {$I BuildTag.inc} {$ENDIF};
  SetupBinVersion = (4 shl 24) + (2 shl 16) + (0 shl 8) + 0;
...
const
  ...
  SetupID: TSetupID = 'Inno Setup Setup Data (4.2.0)';
...
type
  ...
  TSetupLdrExeHeader = packed record
    ...
    OffsetTableOffset: Longint;     { Offset of a TSetupLdr(split_id)OffsetTable record }
    NotOffsetTableOffset: Longint;  { = not OffsetTableOffset }
  end;

  TSetupLdrOffsetTable = packed record
    ID: array[1..12] of Char;       { = SetupLdr(split_id)OffsetTableID }
