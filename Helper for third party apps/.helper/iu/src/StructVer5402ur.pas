unit StructVer5402ur;

interface
uses
  MyTypes, Int64Em, struct5402ur;
...
const
  ...
  ord(struct5402ur.foConfirmOverwrite)                                           ,  {foConfirmOverwrite        }
  ord(struct5402ur.foUninsNeverUninstall)                                        ,  {foUninsNeverUninstall     }
  ord(struct5402ur.foRestartReplace)                                             ,  {foRestartReplace          }
  ord(struct5402ur.foDeleteAfterInstall)                                         ,  {foDeleteAfterInstall      }
  ord(struct5402ur.foRegisterServer)                                             ,  {foRegisterServer          }
  ord(struct5402ur.foRegisterTypeLib)                                            ,  {foRegisterTypeLib         }
  ord(struct5402ur.foSharedFile)                                                 ,  {foSharedFile              }
  {$IF 5402>=3005}
  ord(struct5402ur.foCompareTimeStamp)                                           ,  {foCompareTimeStamp        }
  {$ELSE}
  ord(struct5402ur.foCompareTimeStampAlso)                                       ,  {foCompareTimeStamp        }
  {$IFEND}
  ord(struct5402ur.foFontIsntTrueType)                                           ,  {foFontIsntTrueType        }
  ord(struct5402ur.foSkipIfSourceDoesntExist)                                    ,  {foSkipIfSourceDoesntExist }
  ord(struct5402ur.foOverwriteReadOnly)                                          ,  {foOverwriteReadOnly       }
  ord(struct5402ur.foOverwriteSameVersion)                                       ,  {foOverwriteSameVersion    }
  ord(struct5402ur.foCustomDestName)                                             ,  {foCustomDestName          }
  {$IF 5402>=1325}ord(struct5402ur.foOnlyIfDestFileExists)    {$ELSE}255{$IFEND} ,  {foOnlyIfDestFileExists    }
  {$IF 5402>=2001}ord(struct5402ur.foNoRegError)              {$ELSE}255{$IFEND} ,  {foNoRegError              }
  {$IF 5402>=3001}ord(struct5402ur.foUninsRestartDelete)      {$ELSE}255{$IFEND} ,  {foUninsRestartDelete      }
  {$IF 5402>=3005}ord(struct5402ur.foOnlyIfDoesntExist)       {$ELSE}255{$IFEND} ,  {foOnlyIfDoesntExist       }
  {$IF 5402>=3005}ord(struct5402ur.foIgnoreVersion)           {$ELSE}255{$IFEND} ,  {foIgnoreVersion           }
  {$IF 5402>=3005}ord(struct5402ur.foPromptIfOlder)           {$ELSE}255{$IFEND} ,  {foPromptIfOlder           }
  {$IF 5402>=4000}ord(struct5402ur.foDontCopy)                {$ELSE}255{$IFEND} ,  {foDontCopy                }
  {$IF 5402>=4005}ord(struct5402ur.foUninsRemoveReadOnly)     {$ELSE}255{$IFEND} ,  {foUninsRemoveReadOnly     }
  {$IF 5402>=4108}ord(struct5402ur.foRecurseSubDirsExternal)  {$ELSE}255{$IFEND} ,  {foRecurseSubDirsExternal  }
  {$IF 5402>=4201}ord(struct5402ur.foReplaceSameVersionIfContentsDiffer){$ELSE}255{$IFEND} ,
  {$IF 5402>=4205}ord(struct5402ur.foDontVerifyChecksum)      {$ELSE}255{$IFEND} ,
  {$IF 5402>=5003}ord(struct5402ur.foUninsNoSharedFilePrompt) {$ELSE}255{$IFEND} ,
  {$IF 5402>=5100}ord(struct5402ur.foCreateAllSubDirs)        {$ELSE}255{$IFEND} ,
  {$IF 5402>=5102}ord(struct5402ur.fo32bit)                   {$ELSE}255{$IFEND} ,  {fo32bit                   }
  {$IF 5402>=5102}ord(struct5402ur.fo64bit)                   {$ELSE}255{$IFEND} ,  {fo64bit                   }
  {$IF 5402>=5200}ord(struct5402ur.foExternalSizePreset)      {$ELSE}255{$IFEND} ,  {foExternalSizePreset      }
  {$IF 5402>=5200}ord(struct5402ur.foSetNTFSCompression)      {$ELSE}255{$IFEND} ,  {foSetNTFSCompression      }
  {$IF 5402>=5200}ord(struct5402ur.foUnsetNTFSCompression)    {$ELSE}255{$IFEND} ,  {foUnsetNTFSCompression    }
  {$IF 5402>=5300}ord(struct5402ur.foGacInstall)              {$ELSE}255{$IFEND}    {foGacInstall              }
);

  SetupFileLocationFlagTable: array[0..MySetupFileLocationFlagLast] of byte = (
    ord(struct5402ur.foVersionInfoValid)                                              ,
    ord(struct5402ur.foVersionInfoNotValid)                                           ,
    {$IF 5402>=4010}ord(struct5402ur.foTimeStampInUTC)           {$ELSE}255{$IFEND} ,
    {$IF 5402>=4100}ord(struct5402ur.foIsUninstExe)              {$ELSE}255{$IFEND} ,
    {$IF 5402>=4108}ord(struct5402ur.foCallInstructionOptimized) {$ELSE}255{$IFEND} ,
    {$IF (5402>=4200) AND (5402<5507)}ord(struct5402ur.foTouch){$ELSE}255{$IFEND} ,
    {$IF 5402>=4202}ord(struct5402ur.foChunkEncrypted)           {$ELSE}255{$IFEND} ,
    {$IF 5402>=4205}ord(struct5402ur.foChunkCompressed)          {$ELSE}255{$IFEND} ,
    {$IF 5402>=5113}ord(struct5402ur.foSolidBreak)               {$ELSE}255{$IFEND}
  );

  SetupRegistryOptionTable: array[0..MySetupRegistryOptionLast] of byte = (
  ord(struct5402ur.roCreateValueIfDoesntExist)                                   ,  {roCreateValueIfDoesntExist    }
  ord(struct5402ur.roUninsDeleteValue)                                           ,  {roUninsDeleteValue            }
  ord(struct5402ur.roUninsClearValue)                                            ,  {roUninsClearValue             }
  ord(struct5402ur.roUninsDeleteEntireKey)                                       ,  {roUninsDeleteEntireKey        }
  ord(struct5402ur.roUninsDeleteEntireKeyIfEmpty)                                ,  {roUninsDeleteEntireKeyIfEmpty }
  ord(struct5402ur.roPreserveStringType)                                         ,  {roPreserveStringType          }
  ord(struct5402ur.roDeleteKey)                                                  ,  {roDeleteKey                   }
  ord(struct5402ur.roDeleteValue)                                                ,  {roDeleteValue                 }
  ord(struct5402ur.roNoError)                                                    ,  {roNoError                     }
  ord(struct5402ur.roDontCreateKey)                                              ,  {roDontCreateKey               }
  {$IF 5402>=5100}ord(struct5402ur.ro32bit)                 {$ELSE}255{$IFEND} ,  {ro32bit                       }
  {$IF 5402>=5100}ord(struct5402ur.ro64bit)                 {$ELSE}255{$IFEND}    {ro64bit                       }
);

  SetupDirOptionTable: array[0..MySetupDirOptionLast] of byte = (
  ord(struct5402ur.doUninsNeverUninstall)                                          ,  {doUninsNeverUninstall   }
  ord(struct5402ur.doDeleteAfterInstall)                                           ,  {doDeleteAfterInstall    }
  ord(struct5402ur.doUninsAlwaysUninstall)                                         ,  {doUninsAlwaysUninstall  }
  {$IF 5402>=5200}ord(struct5402ur.doSetNTFSCompression)      {$ELSE}255{$IFEND} ,  {doSetNTFSCompression    }
  {$IF 5402>=5200}ord(struct5402ur.doUnsetNTFSCompression)    {$ELSE}255{$IFEND}    {doUnsetNTFSCompression  }
);

  SetupIniOptionTable: array[0..MySetupIniOptionLast] of byte = (
  ord(struct5402ur.ioCreateKeyIfDoesntExist)                       ,  {ioCreateKeyIfDoesntExist     }
  ord(struct5402ur.ioUninsDeleteEntry)                             ,  {ioUninsDeleteEntry           }
  ord(struct5402ur.ioUninsDeleteEntireSection)                     ,  {ioUninsDeleteEntireSection   }
  ord(struct5402ur.ioUninsDeleteSectionIfEmpty)                    ,  {ioUninsDeleteSectionIfEmpty  }
  ord(struct5402ur.ioHasValue)                                        {ioHasValue                   }
);

  SetupHeaderOptionTable: array[0..MySetupHeaderOptionLast] of byte = (
    ord(struct5402ur.shDisableStartupPrompt),
    {$IF 5402<5310}ord(struct5402ur.shUninstallable)             {$ELSE}255{$IFEND},
    ord(struct5402ur.shCreateAppDir),
    ord(struct5402ur.shAllowNoIcons),
    {$IF (5402>=3003) AND (5402 < 3000)}ord(struct5402ur.shAlwaysRestart) {$ELSE}255{$IFEND},
    ord(struct5402ur.shAlwaysUsePersonalGroup),
    ord(struct5402ur.shWindowVisible),
    ord(struct5402ur.shWindowShowCaption),
    ord(struct5402ur.shWindowResizable),
    ord(struct5402ur.shWindowStartMaximized),
    ord(struct5402ur.shEnableDirDoesntExistWarning),
    ord(struct5402ur.shPassword),
    ord(struct5402ur.shAllowRootDirectory),
    ord(struct5402ur.shDisableFinishedPage),
    ord(struct5402ur.shChangesAssociations),
    ord(struct5402ur.shUsePreviousAppDir),
    ord(struct5402ur.shBackColorHorizontal),
    ord(struct5402ur.shUsePreviousGroup),
    ord(struct5402ur.shUpdateUninstallLogAppName),
    {$IF 5402>=2001}ord(struct5402ur.shUsePreviousSetupType)      {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.shDisableReadyMemo)          {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.shAlwaysShowComponentsList)  {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.shFlatComponentsList)        {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.shShowComponentSizes)        {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.shUsePreviousTasks)          {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.shDisableReadyPage)          {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.shAlwaysShowDirOnReadyPage)  {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.shAlwaysShowGroupOnReadyPage){$ELSE}255{$IFEND},
    {$IF 5402>=2018}ord(struct5402ur.shAllowUNCPath)              {$ELSE}255{$IFEND},
    {$IF 5402>=3000}ord(struct5402ur.shUserInfoPage)              {$ELSE}255{$IFEND},
    {$IF 5402>=3000}ord(struct5402ur.shUsePreviousUserInfo)       {$ELSE}255{$IFEND},
    {$IF 5402>=3001}ord(struct5402ur.shUninstallRestartComputer)  {$ELSE}255{$IFEND},
    {$IF 5402>=3003}ord(struct5402ur.shRestartIfNeededByRun)      {$ELSE}255{$IFEND},
    {$IF 5402>=3008}ord(struct5402ur.shShowTasksTreeLines)        {$ELSE}255{$IFEND},
    {$IF 5402>=4009}ord(struct5402ur.shAllowCancelDuringInstall)  {$ELSE}255{$IFEND},
    {$IF 5402>=4103}ord(struct5402ur.shWizardImageStretch)        {$ELSE}255{$IFEND},
    {$IF 5402>=4108}ord(struct5402ur.shAppendDefaultDirName)      {$ELSE}255{$IFEND},
    {$IF 5402>=4108}ord(struct5402ur.shAppendDefaultGroupName)    {$ELSE}255{$IFEND},
    {$IF 5402>=4202}ord(struct5402ur.shEncryptionUsed)            {$ELSE}255{$IFEND},
    {$IF 5402>=5004}ord(struct5402ur.shChangesEnvironment)        {$ELSE}255{$IFEND},
    {$IF (5402>=5107) AND (NOT DEFINED(UNICODE))}
      ord(struct5402ur.shShowUndisplayableLanguages)
    {$ELSE}255{$IFEND},
    {$IF 5402>=5113}ord(struct5402ur.shSetupLogging)             {$ELSE}255{$IFEND},
    {$IF 5402>=5201}ord(struct5402ur.shSignedUninstaller)        {$ELSE}255{$IFEND},
    {$IF 5402>=5308}ord(struct5402ur.shUsePreviousLanguage)      {$ELSE}255{$IFEND},
    {$IF 5402>=5309}ord(struct5402ur.shDisableWelcomePage)       {$ELSE}255{$IFEND},
    {$IF 5402>=5500}ord(struct5402ur.shCloseApplications)       {$ELSE}255{$IFEND},
    {$IF 5402>=5500}ord(struct5402ur.shRestartApplications)       {$ELSE}255{$IFEND},
    {$IF 5402>=5500}ord(struct5402ur.shAllowNetworkDrive)       {$ELSE}255{$IFEND}
  );

  SetupRunOptionTable: array[0..MySetupRunOptionLast] of byte = (
    ord(struct5402ur.roShellExec),
    ord(struct5402ur.roSkipIfDoesntExist),
    {$IF 5402>=2001}ord(struct5402ur.roPostInstall)         {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.roUnchecked)           {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.roSkipIfSilent)        {$ELSE}255{$IFEND},
    {$IF 5402>=2001}ord(struct5402ur.roSkipIfNotSilent)     {$ELSE}255{$IFEND},
    {$IF 5402>=2008}ord(struct5402ur.roHideWizard)          {$ELSE}255{$IFEND},
    {$IF 5402>=5110}ord(struct5402ur.roRun32Bit)            {$ELSE}255{$IFEND},
    {$IF 5402>=5110}ord(struct5402ur.roRun64Bit)            {$ELSE}255{$IFEND},
    {$IF 5402>=5200}ord(struct5402ur.roRunAsOriginalUser)   {$ELSE}255{$IFEND}
  );
...
//    {$IF 5402<=4106} // since the main installer exe is not unpacked, there's no need for its size
...
{$IFEND}
{$IF 5402>=5306}
    UninstallDisplaySize := SetInteger64(osh.UninstallDisplaySize);
{$ELSE}
    UninstallDisplaySize := SetInteger64(0);
{$IFEND}
  end;
...
{$ELSE}
    ...
    InfoAfterText                 :='';
{$IFEND}
{$IF (5402>=4202) AND not Defined(UNICODE)}
...
{$IFEND}
    ...
    RightToLeft                   :={$IF 5402>=5203} ole.RightToLeft {$ELSE} false {$IFEND};
  end;
{$ELSE}
begin
{$IFEND}
...
begin
  with de do begin
    ...
    Tasks               :='';
    {$IFEND}
    Languages           := {$IF 5402>=4001} ode.Languages {$ELSE} '' {$IFEND};
    Check               := {$IF 5402>=3008} ode.Check {$ELSE} '' {$IFEND};
{$IF 5402>=4100}
...
{$ELSE}
    ...
    BeforeInstall       := '';
{$IFEND}
    Attribs             := {$IF 5402>=2011} ode.Attribs {$ELSE} 0 {$IFEND}; 
...
begin
  with ie do begin
    ...
    Tasks               := '';
    {$IFEND}
    Languages           := {$IF 5402>=4001} oie.Languages {$ELSE} '' {$IFEND};
    Check               := {$IF 5402>=3008} oie.Check {$ELSE} '' {$IFEND};
{$IF 5402>=4100}
    AfterInstall        := oie.AfterInstall;
    BeforeInstall       := oie.BeforeInstall;
{$ELSE}
