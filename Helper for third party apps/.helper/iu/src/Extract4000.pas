...
interface
uses
  Int64Em, Compress, Struct, Extract, FileClass;
...
type
  ...
  TDecompressExtraData = record
    ...
    BytesLeft, Size, BytesWritten: Integer64;
...
implementation
uses
  SysUtils, zlib, bzlib, Msgs, CmnFunc2, PathFunc, Main, MyTypes, Windows;
...
begin
  ...
  with PDecompressExtraData(ExtraData)^ do begin
    Left := MaxBytes;
    if (BytesLeft.Hi = 0) and (BytesLeft.Lo < Left) then
      Left := BytesLeft.Lo;
    Result := Left;
...
begin
  with PDecompressExtraData(ExtraData)^ do begin
    Inc64(BytesWritten, BufSize);
    if Compare64(BytesWritten, Size) > 0 then
      SourceIsCorrupted;
...
begin
  ...
  if (Compare64(DecompressData.BytesWritten, FL.OriginalSize) <> 0) or (Adler32 <> FL.CRC) then
    SourceIsCorrupted;
