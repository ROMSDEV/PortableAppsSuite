...
procedure InflateBlockReadBegin(var F: File; var Data: TDeflateBlockReadData);
...
function adler32(adler: Longint; const buf; len: Cardinal): Longint; far;
function inflateInit_ (var strm: TZStreamRec; version: PChar;
  recsize: Integer): Integer; far;
function inflate(var strm: TZStreamRec; flush: Integer): Integer; far;
function inflateEnd(var strm: TZStreamRec): Integer; far;
...
function _malloc(size:cardinal):pointer;
begin
  GetMem(Result,size);
end;
...
begin
  ...
  if FilePos(F) + SizeOf(HdrCRC) + SizeOf(Hdr) > FileSize(F) then
    raise ECompressDataError.Create(SZlibDataError);
  BlockRead(F, HdrCRC, SizeOf(HdrCRC));
  BlockRead(F, Hdr, SizeOf(Hdr));
  if HdrCRC <> GetCRC32(Hdr, SizeOf(Hdr)) then
    raise ECompressDataError.Create(SZlibDataError);
  if FilePos(F) + Hdr.StoredSize > FileSize(F) then
    raise ECompressDataError.Create(SZlibDataError);
