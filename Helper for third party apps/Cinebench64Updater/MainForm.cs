namespace AppUpdater
{
    ...
    public partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            Icon = Resources.CinebenchUpdater;
        }
            ...
            try
            {
                ...
                using (var archive = ZipFile.OpenRead(_transfer.FilePath))
                    foreach (var ent in archive.Entries)
                        try
                        {
                            if (!first)
                            {
                                ...
                                if (ent.FullName.EndsWith("/"))
                                    dir = ent.FullName.TrimEnd('/');
                            }
                            if (!Path.HasExtension(ent.FullName))
                                continue;
                            var entPath = ent.FullName;
                            if (!string.IsNullOrEmpty(dir) && entPath.StartsWithEx(dir))
                                entPath = entPath.Substring(dir.Length).TrimStart('/');
                        ...
                        {
                            Log.Write(ex);
                            if (ent.FullName.ContainsEx("cb_ranking"))
                                continue;
                            entry = ent.FullName.Split('/').Skip(string.IsNullOrEmpty(dir) ? 0 : 1).Join('\\');
                            throw;
                        }
                ...
                appDir = PathEx.Combine(Resources.AppPath);
                if (File.Exists(appDir))
                    File.SetLastWriteTime(appDir, DateTime.Now);
                e.Result = true;
            }
